...
function getMetadata (opts, dir, cb) {
  ...
  if (!opts.version) props.push(['dependencies.tr-electron', 'devDependencies.tr-electron'])
  ...
  getPackageInfo(props, dir, function (err, result) {
    if (err) {
      ...
      props.pop()
      props.push(['dependencies.tr-electron-prebuilt', 'devDependencies.tr-electron-prebuilt'])
      getPackageInfo(props, dir, function (err, result) {
        if (err) return cb(err)
        return inferNameAndVersionFromInstalled('tr-electron-prebuilt', opts, result, cb)
      })
    } else {
      return inferNameAndVersionFromInstalled('tr-electron', opts, result, cb)
    }
  })
}
...
function createSeries (opts, archs, platforms) {
  var tempBase = path.join(opts.tmpdir || os.tmpdir(), 'tr-electron-packager')
    ...
    return (callback) => {
      common.downloadElectronZip(combination, (err, zipPath) => {
        ...
        function createApp (comboOpts) {
          ...
          console.error(`Packaging app for platform ${platform} ${arch} using tr-electron v${version}`)
          series([
            function (cb) {
