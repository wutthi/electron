...
window.open = function (url, frameName, features) {
  var feature, guestId, i, j, len, len1, name, options, additional_features, ref1, ref2, value, int_val
  if (frameName == null) {
    frameName = ''
  }
  if (features == null) {
  ...
  var getTypedFeatureValue = function (featureValue) {
  ...
  for (i = 0, len = ref1.length; i < len; i++) {
    ...
    // Keep additional features in a separate object
    if(value == undefined) {
        ref2 = feature.split(/\s*-/)
        name = ref2[0]
        value = getTypedFeatureValue(ref2[1])
        additional_features[name] = value
    } else {
  ...
  guestId = ipcRenderer.sendSync('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_OPEN', url, frameName, disposition, options, additional_features)
  if (guestId) {
